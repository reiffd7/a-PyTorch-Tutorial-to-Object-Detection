...
class SSD300(nn.Module):
    ...
    def freeze_base_layers(self, freeze_up_to="conv4_3"):
        """
        Freeze layers in the base network up to a specified layer.
        :param freeze_up_to: name of the layer up to which to freeze (inclusive)
        """
        layers_to_freeze = [
            "conv1_1",
            "conv1_2",
        ...
        for name, param in self.base.named_parameters():
            ...
            if name.startswith(freeze_up_to):
                break

    def freeze_aux_layers(self, layers_to_freeze):
        """
        Freeze specified auxiliary convolutional layers.
        :param layers_to_freeze: list of layer names to freeze
        """
        for name, param in self.aux_convs.named_parameters():
            if any(name.startswith(layer) for layer in layers_to_freeze):
                param.requires_grad = False

    def freeze_pred_layers(self, layers_to_freeze):
        """
        Freeze specified prediction convolutional layers.
        :param layers_to_freeze: list of layer names to freeze
        """
        for name, param in self.pred_convs.named_parameters():
            if any(name.startswith(layer) for layer in layers_to_freeze):
                param.requires_grad = False
    ...
    def print_trainable_parameters(self):
        """
        Print the names and shapes of trainable parameters in the model.
        """
        ...
        for name, param in self.named_parameters():
            if param.requires_grad:
                ...
                print(f"NOT TRAINABLE: {name}: {param.shape} ")
